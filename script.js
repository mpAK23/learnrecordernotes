...
const LEVELS = {
    soprano: {
        1: ["C4", "D4", "E4", "F4", "G4", "A4", "B4", "C5", "D5"],
        2: ["F4", "E4", "D4", "C4", "A#4", "F#4", "G#4", "C#5", "D#5", "F#5"]
    },
    alto: {
        1: ["F4", "G4", "A4", "B4", "C5", "D5", "E5", "F5", "G5"],
        2: ["A#4", "G#4", "C#5", "D#5", "F#5", "G#5", "A#5"]
    },
    bass: {
        1: ["F2", "G2", "A2", "B2", "C3", "D3", "E3", "F3", "G3"],
        2: ["A#2", "G#2", "C#3", "D#3", "F#3", "G#3", "A#3"]
    }
};
...
function setDifficulty(lvl) {
    // If clicking the current highest level, visually "un-highlight" that level (drop down by 1)
    if (appState.difficulty === lvl) {
        if (lvl > 1) {
            appState.difficulty = lvl - 1;
        } else {
            return; // Can't go below Level 1
        }
    } else {
        // Clicking a new target level
        appState.difficulty = lvl;
    }

    // Additive highlighting: highlight all buttons up to the current difficulty
    if (btnLvl1) btnLvl1.classList.toggle('active', appState.difficulty >= 1);
    if (btnLvl2) btnLvl2.classList.toggle('active', appState.difficulty >= 2);
    if (btnLvl3) btnLvl3.classList.toggle('active', appState.difficulty >= 3);
...
function setInstrument(inst) {
    ...
    if (appState.mode === 'explore') {
        ...
        if (typeof playNote === 'function') playNote(firstNote);
...
function renderStaff() {
    ...
    if (appState.instrument === 'bass') {
        clefTxt.setAttribute("y", "62"); // Align with F line (Y=60)
        clefTxt.setAttribute("font-size", "35");
        clefTxt.textContent = "𝄢";
    } else {
        clefTxt.setAttribute("y", "82"); // Align with G line (Y=80)
        clefTxt.setAttribute("font-size", "35");
        clefTxt.textContent = "𝄞";
    }
...
function nextQuizQuestion() {
    ...
    // Split pool into naturals and accidentals for weighted selection
    const naturals = pool.filter(n => !n.includes('#'));
    const accidentals = pool.filter(n => n.includes('#'));

    let rnd;
    if (naturals.length > 0 && accidentals.length > 0) {
        // 80% chance for a natural note
        if (Math.random() < 0.8) {
            rnd = naturals[Math.floor(Math.random() * naturals.length)];
        } else {
            rnd = accidentals[Math.floor(Math.random() * accidentals.length)];
        }
    } else {
        // Fallback if one category is missing (e.g. Level 1 usually has no accidentals)
